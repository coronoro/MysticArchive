...
class InventoryController(Controller):
    ...
    async def get_inventory(self, db_session: "AsyncSession", db_engine: "AsyncEngine",
                            inventory_id: UUID) -> Inventory:
        return await db_session.query(Inventory).get(inventory_id)

    @post('/{inventory_id:uuid}/')
    async def add_card(self, db_session: "AsyncSession", db_engine: "AsyncEngine", inventory_id: UUID,
                       card_id: UUID) -> Inventory:
